...
public class McpTool {
  ...
  public static void main(String[] args) {
    ...
    opts.addOption(
        Option.builder("t").longOpt("tools").desc("List all available MCP tools in JSON").build());
    opts.addOption(
        Option.builder("u")
            .longOpt("url")
            .hasArg()
            ...
            .build());
    opts.addOption(Option.builder("h").longOpt("help").desc("Show this help message").build());
    ...
    try {
      ...
      HttpClientSseClientTransport transport =
          HttpClientSseClientTransport.builder(baseUrl).build();
      McpSyncClient client = McpClient.sync(transport).build();

      // 3) initialize (ping)
      McpSchema.InitializeResult init = client.initialize();
      if (!listTools) {
        System.out.printf("Ping successful, protocolVersion=%s%n", init.protocolVersion());
        return;
      }
